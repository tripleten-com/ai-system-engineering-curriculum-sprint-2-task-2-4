...
File:              tests/contract/test_authorization_extension.py
...
# Assessed: a fresh starter composes the unrestricted policy and is supposed to
# fail these. Runtime: the exclusion checks read the running retrieval API.
pytestmark = [pytest.mark.runtime, pytest.mark.assessed]

CHOICES = ("tenant_boundary", "role_classification")
...
def selected_filter_type() -> str:
    """Return the recorded filter choice or fail with actionable guidance."""
    ...
    choice = answers.get("selected_filter_type", "") if isinstance(answers, dict) else ""
    if choice not in CHOICES:
        pytest.fail(
            "answers.selected_filter_type must be either tenant_boundary or "
            f"role_classification; found {choice!r}"
        )
    return str(choice)
...
def test_the_composed_policy_actually_restricts_something() -> None:
    """The starter's unrestricted policy must have been replaced."""
    selected_filter_type()
    constraint = _constrain(OWN_TENANT, AccessTier.STANDARD)
    assert constraint.restricts, (
        "the composed policy restricts nothing, so the application is still running the "
        "supplied unrestricted constraint; return your provider from "
...
def test_the_recorded_choice_matches_the_implemented_mechanism() -> None:
    ...
    if choice == "tenant_boundary":
        ...
        assert standard.access_tiers is None, (
            "a tenant_boundary constraint restricts tenancy only; it must not also "
        ...
        assert standard.access_tiers == (AccessTier.STANDARD,), (
            f"a standard caller must be limited to the standard tier; found "
            f"{standard.access_tiers!r}"
        )
        assert cleared.access_tiers is not None
        assert set(cleared.access_tiers) == {AccessTier.STANDARD, AccessTier.RESTRICTED}, (
            f"a restricted caller reads both tiers; found {cleared.access_tiers!r}"
        )
