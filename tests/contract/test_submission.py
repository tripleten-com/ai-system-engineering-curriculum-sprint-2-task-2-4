...
def valid_answers(choice: str = "role_classification") -> dict[str, object]:
    """Return a complete answer sheet naming one supported mechanism."""
    return {"answers": {"selected_filter_type": choice}}
...
def test_blank_template_fails_with_field_address(tmp_path: Path) -> None:
    ...
    with pytest.raises(SubmissionError, match="answers.selected_filter_type"):
        validate_submission(root / "submission.yaml", SCHEMA)


def test_unsupported_choice_is_rejected(tmp_path: Path) -> None:
    """A third mechanism is not on offer in this Task."""
    root = _task_root(tmp_path, yaml.safe_dump(valid_answers("multi_attribute_policy_engine")))

    with pytest.raises(SubmissionError, match="selected_filter_type"):
        validate_submission(root / "submission.yaml", SCHEMA)
...
def test_unexpected_answer_field_is_rejected(tmp_path: Path) -> None:
    ...
    mapping["filter_rationale"] = "no free text is assessed in this Task"
...
def test_student_editable_paths_and_prefixes_are_permitted() -> None:
    ...
    validate_changed_paths(["src/api/extensions/authorization.py"])
    validate_changed_paths(["tests/student/test_my_boundary.py"])

    with pytest.raises(SubmissionError, match="src/domain/access.py"):
        validate_changed_paths(["src/domain/access.py"])

    with pytest.raises(SubmissionError, match="src/ports/retriever.py"):
        validate_changed_paths(["src/ports/retriever.py"])

    with pytest.raises(SubmissionError, match="tests/contract"):
        validate_changed_paths(["tests/contract/test_authorization_extension.py"])
...
def test_public_entrypoint_reports_an_incomplete_answer_sheet(
    ...
    assert "answers.selected_filter_type is incomplete" in capsys.readouterr().err
