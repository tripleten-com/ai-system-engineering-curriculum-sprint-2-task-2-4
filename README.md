# Coldline Task 2.4 — Authorization-aware `Retriever` extension

[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://codespaces.new/tripleten-com/ai-system-engineering-curriculum-sprint-2-task-2-4/tree/main)
...
| `poe authorization` | Run this Task's authorization checks |
...
For Task 2.4, `poe verify` rebuilds and starts the stack, ingests the supplied corpus, then runs
readiness, smoke tests, the end-to-end exception workflow, the answer-sheet checks, the
authorization checks, and your own tests under `tests/student/`.

`poe verify` begins with `poe start`, which is `up --build --wait`. Your policy runs inside the
...
└── tests/
    ...
    ├── contract/        Interface, retrieval, authorization, and repository checks
...
Use the Task 2.4 lesson to decide what to do. This README covers local setup and repository
orientation.

1. `README.md` — local setup, commands, and permitted changes.
2. [`docs/student/task-2-4-contract.md`](docs/student/task-2-4-contract.md) — the two choices,
   what each must restrict, and where each check looks.
3. `src/domain/access.py` — `AccessConstraint` and its three distinguishable states.
4. `src/api/extensions/authorization.py` — the file you implement.
5. `src/api/extensions/wiring.py` — where you compose it.
6. `src/adapters/retriever/postgres_hybrid.py` — the supplied adapter that applies your
   constraint inside both query arms.
7. [`docs/fidelity/Retriever.md`](docs/fidelity/Retriever.md) — what enforcing a constraint
   against an asserted caller identity does and does not establish.
...
Task 2.4 asks you to implement **one** bounded authorization mechanism behind the published
...
A constraint that denies everything is not a pass. It excludes out-of-scope content and the
caller's own content alike, and the permitted-retrieval check requires the caller's own document
back.
